...
from waveview.adapter import WaveViewAdapter
...
def reverse_process_event_and_updatedb(
    engine, table, event_db, event_wo, *, dry_run=False
):
    """
    ...
    if event_wo is None:
        ...
        if dry_run:
            ...
            ok = ops.restore_event(engine, table, eventid, eventtype_wo, operator_wo)
            if ok:
...
class SimpleEventVisitor(SingleInstance):
    ...
    def reverse_process_events(self, events, start, end, eventtype=None):
        ...
        for event, wo_event in dbquery.reverse_filter_exact(
            ...
            reverse_process_event_and_updatedb(self.engine, self.table, event, wo_event)
...
def sync_webobs_and_bulletin(engine, table, events, *, dry_run=False):
    ...
    for event_wo, event_db in query.filter_exact(engine, table, events):
        ...
        event = process_event_and_updatedb(engine, event_wo, dry_run=dry_run)
        waveview.update_event(event)


def sync_bulletin_and_webobs(
    engine, table, events, start, end, *, eventtype=None, dry_run=False
):
    """
...
def _execute_action(
    ...
    *,
    ...
    logger.info(
        "Event meta data: [eventdate: %s, eventid: %s, "
    ...
    if action == WebObsAction.WEBOBS_UPDATE_EVENT:
        ...
        if eventid is None:
            raise ValueError(
                "Action %s requires eventid value not None",
        ...
        if eventtype is None:
            raise ValueError(
                "Action %s requires eventtype value not None",
                WebObsAction.WEBOBS_RESTORE_EVENT,
            )
...
def update_event(
    ...
    *,
    ...
    _execute_action(
        engine,
        table,
        WebObsAction.WEBOBS_UPDATE_EVENT,
        eventdate=eventdate,
        eventid=eventid,
        sc3id=sc3id,
        eventtype=eventtype,
