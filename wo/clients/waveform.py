...
class WaveformClient(object):
    ...
    def __init__(self, seedlink_host='localhost', seedlink_port=18000,
                 arclink_host='localhost', arclink_port=18001,
                 timeout=5, use_client=None):
    ...
    def get_waveforms_via_seedlink(self, network, station, location,
    ...
    def get_waveforms_via_arclink(self, network, station, location,
                                  channel, starttime, endtime):
        """
        Fetch waveform data via ArcLink.
        """
        logger.debug('Using ArcLink client.')
        client = ArcLinkClient('client@cendana15.com', host=self.arclink_host,
                               port=self.arclink_port, timeout=self.timeout)
        stream = client.get_waveforms(
            network, station, location, channel, starttime, endtime)
        return stream
...
def get_waveforms(starttime, endtime):
    ...
    if abs(delta) < TIME_BUFFER_OFFSET:
        for sta in SE_STATIONS:
            ...
            try:
                logger.info(
                    'Fetching waveform data for station '
                    '%s (fast mode)...', station)
        ...
        for sta in STATIONS:
            ...
            try:
                logger.info(
                    'Fetching waveform data for station %s...', station)
